...
| Hooks | Sync subscription options | Compat sync hooks force `async:false`, `renderAttemptCleanup:true`, `defer:false`. | Non-compat sync hooks только force-ят `async:false`, не force-ят cleanup/defer. | `src/orm/Compat/hooksCompat.js:355`, `src/react/useSub.ts:264`, `src/react/convertToObserver.js:37` | Влияет на route/tab transitions и Suspense behavior. LMS массово использует `useDoc`, `useQuery`, `useBatch`; route/layout/stage screens зависят от sync subscription shape. | Нужны runtime smoke-тесты без compat по route transitions, tab switches и screens с `useBatch()`. |
| Hooks | Batch subscription options | Batch hooks всегда используют `async:false`, `batch:true`, `renderAttemptCleanup:true`, `defer:false`. | Batch options не зависят от `isCompatEnv()` в текущем коде. | `src/orm/Compat/hooksCompat.js:346` | `useBatch(` около 288 совпадений. | Проверить, какие batch expectations останутся без compat. |
...
| Hooks behavior без compat | По коду hooks API остаётся экспортированным, но sync behavior меняется: в non-compat нет forced `renderAttemptCleanup` и `defer:false`. Teamplay tests покрывают отдельные mechanics, но LMS runtime без compat не проверялся. | `src/orm/Compat/hooksCompat.js:355`, `src/react/useSub.ts:264`, `src/react/convertToObserver.js:37`, `src/react/renderAttemptDestroyer.ts`. | Нужны smoke-тесты/ручной прогон без `TEAMPLAY_COMPAT=1` по route transitions, screens с `useBatch()`, stage player/dashboard и forms. |
