...
| DataTree | Immediate subpath writes after add | Compat может использовать local raw data как source of truth, пока ShareDB doc.data пустой. | Non-compat считает doc missing, если snapshot не загружен. | `src/orm/dataTree.js:351`, `src/orm/dataTree.js:397` | Важно для server/model code, который сразу после add пишет subpaths. | Найти последовательности `add` -> immediate `set/setDiff`. |
