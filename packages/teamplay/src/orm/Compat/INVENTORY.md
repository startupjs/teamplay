...
| Runtime | Root proxy `connection` / `root` | Root proxy в compat дополнительно отдаёт `connection`; `root` явно пробрасывается. | Эти специальные proxy-getters не добавляются. | `src/orm/getSignal.ts:100`, `test/signalCompat.js:397` | Есть прямые `$root.connection` / `this.root.connection` в rich-text flows: `main/stages/fields/richText/FRichTextEdit.web.js`, `main/stages/MT_materials/edit/MTEditContent/index.js`, `modelCards/CardModel/BaseCardModel.js`; `model.root.get('_session.userId')` есть в `helpers/filters.js`. В deps: `@dmapper/rich-text-editor`, `@dmapper/versioning` tests. | Уточнить, какие обращения к `.root` являются Teamplay model API, а какие CSS/component `.root`. |
