...
| Runtime | Root proxy `connection` / `root()` | Compat больше не отдаёт ShareDB connection через root proxy; root traversal переводится на явный метод `root()`, а `root` без вызова остаётся обычным child-path. | `root()` доступен на базовом `Signal`; специальных proxy-getters для `root` нет. ShareDB connection доступен через explicit `getConnection()`. | `src/orm/SignalBase.ts`, `src/orm/getSignal.ts`, `test/signalCompat.js`, `test/signalMetadata.js` | `$root.connection` / `this.root().connection` в rich-text flows надо мигрировать на `getConnection()`. Старые `model.root.get('_session.userId')` должны стать `model.root()._session.userId.get()`. В deps: `@dmapper/rich-text-editor`, `@dmapper/versioning` tests. | Уточнить, какие обращения к `.root` являются Teamplay model API, а какие CSS/component `.root`. |
...
| Runtime / policy | Private mutations | Private mutations разрешены. Server global-root private writes warn once per collection. | То же поведение. | `src/orm/privateData.js` | Request-scoped `_session` / `_page` writes остаются легитимными. | Проверять warning-и на случайные private writes через global `$` на сервере. |
...
| Subscription API | `.query(collection, params)` | Removed from compat signal API. | Use `sub($collection, params)` / `sub($collection, aggregateParams)` instead. | `src/orm/sub.ts` | LMS target branch migrated away from `model.query(...)`. | Dependency scan may still contain historical usages. |
| Subscription API | `.subscribe(...items)` / `.unsubscribe(...items)` | Removed from compat signal API. | Use explicit `sub()` / `unsub()` flows; no Racer-style subscribe-many helper. | `src/orm/sub.ts` | LMS target branch migrated away from direct signal/root subscribe helpers. | Server flows should use `await sub($doc)` / `await sub($collection, params)` and `unsub(...)`. |
| Subscription API | `.fetch()` / `.unfetch()` | Removed from compat signal API. | Use explicit transport mode: `sub($doc, { mode: 'fetch' })`, `sub($collection, params, { mode: 'fetch' })`, cleanup через `unsub($signal)`. | `src/orm/sub.ts` | LMS target branch migrated away from direct signal fetch helpers. | Отличить transport-level ShareDB `doc.fetch/unfetch` internals от публичного signal API. |
...
| Mutators | `setDiffDeep(path?, value)` | Compat recursive Racer-like diff через compat mutators; выполняется внутри runtime batch. Path overload/ref behavior остаются compat-only. | Base `Signal.setDiffDeep(value)` теперь работает на текущем signal без path overload. | `src/orm/SignalBase.ts`, `src/orm/Compat/SignalCompat.js`, `test/signalConvenienceMethods.js` | `.setDiffDeep(` около 92 совпадений; routing, permissions patches, forms. | Path-first вызовы переписывать на child traversal; проверить recursive delete/set behavior. |
...
| Events | Custom events | `emit`, `useOn`, `useEmit` экспортируются; custom events работают независимо от model events. | Экспорт есть тот же, custom events остаются в core `orm/events`. | `src/orm/events.js`, `src/index.ts:227` | `useOn` используется в LMS, например stage/store UI. | Отличить custom events от model `change/all`. |
...
| `start/stop`, `ref/refExtra/refIds/removeRef` | `start/stop` реально используются в virtual fields/stages. `ref/removeRef` используются для session/user/tenant/tutoringSession/fullscreen/local UI bindings. `refExtra/refIds` в LMS/deps не найдены в targeted scan. | LMS: `clientHelpers/hooks/useInitVirtualFields.js`, `clientHelpers/hooks/useInitVirtualStagesAndSettings.js`, `Root/useGlobalInit.js`, `main/Layout/Tutoring/index.js`, `v5/apps/main/Layout/Tutoring/index.js`, `components/Media/index.js`, `main/components/FilterV2/index.js`, `main/components/Event/index.js`. Tests: `test/signalCompat.js`, `test/rootScopedRefsAndEvents.js`. | Для `.start/.stop` много false positives от media/animation/domain methods; для финального списка нужны typed/runtime callsites. |
