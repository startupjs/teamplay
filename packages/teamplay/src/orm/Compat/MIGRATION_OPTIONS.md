...
| `query()` / imperative subscribe/fetch | Требует отдельного решения: API surface и readiness сильно отличаются от простого traversal. | Racer-like query adapter вероятно нужен дольше остальных. | Переписать server/client flows на non-compat query primitives. |
