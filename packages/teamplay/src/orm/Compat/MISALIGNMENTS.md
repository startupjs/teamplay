...
|---|---|---|---|
| Imperative query API: `.query()`, `.subscribe()`, `.unsubscribe()`, `.fetch()`, `.unfetch()` | Высоко. Старый scan показывал около `1249` `.query(` в LMS и usage в `@dmapper/*` / `@startupjs/*`. Много server/cron/model flows. | Держать временный adapter для LMS/deps. Параллельно переводить flows на explicit non-compat primitives: `sub()`, `unsub()`, `{ mode: 'fetch' }`, `Promise.all(...)`. | Это не только синтаксис: compat query API ещё включает shorthand params, root subscribe-many/callback shape и Racer-style method surface. |
