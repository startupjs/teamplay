...
| `src/react/useSub.ts` | Default `defer`, `compatAttemptCleanup`, batch/async/Suspense behavior и reuse предыдущего signal. |
...
| Query helper hooks: ids/doc variants | `useQueryIds`, `useQueryDoc`, batch/async variants строятся поверх compat query hooks, сортируют по входному ids list, используют `_id ?? id`, default `$sort: { createdAt: -1 }` для doc. | В non-compat core таких helpers может не быть или их shape может быть другим. | Эти helpers есть в LMS/deps и часто используются в business hooks. | Держать helpers в adapter. Для target решить, какие helpers являются product API, а какие refactor to local project helpers. | Это удобные LMS-era abstractions, но не обязательно primitives Teamplay core. |
