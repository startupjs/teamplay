...
function buildTeamplayEnvContent (models, options = {}) {
  ...
  for (const [pattern, parts] of Object.entries(models)) {
    ...
    if (isCollectionPattern(pattern)) {
      ...
      const docParts = models[docPattern] || []
      const docModelPart = docParts.find(part => part.type === 'model')

      const collectionModel = modelPart ? addImport(modelPart.value, 'CollectionModel') : null
      const documentModel = docModelPart ? addImport(docModelPart.value, 'DocumentModel') : null

      if (schemaPart) {
        const schemaImport = addImport(schemaPart.value, 'Schema')
        ...
        const docModelType = buildModelType({
          modelImport: documentModel,
          fallbackValue: 'typeof Signal',
          docType
        })
        if (docModelType.declaration) helperTypes.push(docModelType.declaration)

        collectionLines.push(
          `    ${JSON.stringify(collectionName)}: JsonSchemaSpec<typeof ${schemaImport}, ${collectionModel ? `typeof ${collectionModel}` : 'typeof Signal'}, ${docModelType.reference}>`
        )
      } else {
        collectionLines.push(
          `    ${JSON.stringify(collectionName)}: CollectionSpec<unknown, ${collectionModel ? `typeof ${collectionModel}` : 'typeof Signal'}, ${documentModel ? `typeof ${documentModel}` : 'typeof Signal'}>`
        )
      }
    } else if (modelPart && pattern.includes('*')) {
      const modelImport = addImport(modelPart.value, 'PathModel')
      nestedLines.push(`    ${JSON.stringify(pattern)}: typeof ${modelImport}`)
    }
  }

  if (!collectionLines.length && !nestedLines.length && !fieldLines.length) {
    return [
  ...
  return [
    '// This file is generated by TeamPlay. Do not edit manually.',
    '/* eslint-disable */',
    "import { Signal, type CollectionSpec, type FromJsonSchema, type JsonSchemaSpec, type SignalChild } from 'teamplay'",
    ...declarations,
    '',
    ...helperTypes,
    helperTypes.length ? '' : null,
    "declare module 'teamplay' {",
    collectionLines.length ? '  interface TeamplayCollections {' : null,
    ...collectionLines,
    collectionLines.length ? '  }' : null,
    nestedLines.length ? '  interface TeamplayModels {' : null,
    ...nestedLines,
    nestedLines.length ? '  }' : null,
...
function buildModelType ({ modelImport, fallbackValue }) {
  return { reference: modelImport ? `typeof ${modelImport}` : fallbackValue }
}
...
function isJsonSchemaKeyword (key) {
  return [
    '$id', '$schema', '$comment', 'type', 'properties', 'items', 'required', 'enum',
    'const', 'additionalProperties', 'patternProperties', 'description', 'title',
    'default', 'errorMessage', 'validators', 'collection', 'format', 'minimum',
    'maximum', 'minLength', 'maxLength', 'minItems', 'maxItems', 'uniqueItems',
    'input', 'label', 'options', 'placeholder', 'disabled', 'mimeTypes', 'pattern',
    'min', 'max'
  ].includes(key)
}
