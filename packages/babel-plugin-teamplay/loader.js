...
function normalizeOptions (options = {}) {
  return {
    ...
    pluginTypes: normalizePluginTypes(options.pluginTypes),
    featuresType: typeof options.featuresType === 'string' && options.featuresType ? options.featuresType : undefined,
...
function normalizePluginTypes (pluginTypes = []) {
  if (!Array.isArray(pluginTypes)) return []
  const seen = new Set()
  const res = []
  for (const pluginType of pluginTypes) {
    if (!pluginType || typeof pluginType !== 'object') continue
    if (typeof pluginType.importPath !== 'string' || !pluginType.importPath) continue
    const key = pluginType.importPath
    if (seen.has(key)) continue
    seen.add(key)
    res.push({
      name: typeof pluginType.name === 'string' && pluginType.name ? pluginType.name : undefined,
      importPath: pluginType.importPath,
      optionsType: typeof pluginType.optionsType === 'string' && pluginType.optionsType
        ? pluginType.optionsType
        : undefined
    })
  }
  return res
...
function buildTeamplayEnvContent (models, options = {}) {
  ...
  const pluginImportLines = []
  const pluginOptionLines = []
  const featureTypeLines = []
  let hasPrivateCollections = false
  let counter = 0
  ...
  for (const [pattern, parts] of Object.entries(models)) {
    if (!pattern) continue
    const privateCollectionPattern = isPrivateCollectionPattern(pattern)
    if (privateCollectionPattern) hasPrivateCollections = true
    ...
    if (privateCollectionPattern && schemaPart) {
      const collectionName = pattern
      const privateType = `__${toPrivateTypeName(collectionName)}Private`
      const fieldsType = `__${toPrivateTypeName(collectionName)}Fields`
      helperTypes.push(`type ${privateType} = FromJsonSchema<typeof ${schemaImport}>`)
      schemaModuleLines.push(buildSchemaDefaultInterface(
        getRelativeModuleSpecifier(typesDir, schemaPart.value),
        toPrivateTypeName(collectionName),
        privateType
      ))

      const schemaDocs = extractSchemaDocs(schemaPart.value)
      if (schemaDocs.fields.length) {
        helperTypes.push(buildFieldsInterface(fieldsType, privateType, [collectionName], schemaDocs.fields))
        fieldLines.push(`    ${JSON.stringify(collectionName)}: ${fieldsType}`)
      }
    } else if (isCollectionPattern(pattern) && schemaPart) {
      const collectionName = pattern
  ...
  if (options.featuresType) {
    featureTypeLines.push(`type __TeamplayFeatures = ${options.featuresType}`)
  }

  for (const pluginType of options.pluginTypes) {
    pluginImportLines.push(`import ${JSON.stringify(pluginType.importPath)}`)
    if (pluginType.name && pluginType.optionsType) {
      pluginOptionLines.push(`    ${JSON.stringify(pluginType.name)}: ${pluginType.optionsType}`)
    }
  }
  ...
  if (
    ...
    !fieldLines.length &&
    !pluginImportLines.length &&
    !pluginOptionLines.length &&
    !featureTypeLines.length
  ) {
    return [
  ...
  return [
    '// This file is generated by TeamPlay. Do not edit manually.',
    '/* eslint-disable */',
    buildTeamplayTypeImportLine({ manifestLines, helperTypes, fieldLines, hasPrivateCollections }) || "import 'teamplay'",
    ...pluginImportLines,
    ...declarations,
    '',
    ...
    hasPrivateCollections ? '  interface TeamplayPrivateCollections extends PrivateCollectionsFromManifest<__TeamplayModelManifest> {}' : null,
    manifestLines.length ? '  interface TeamplayModels extends PathModelsFromManifest<__TeamplayModelManifest> {}' : null,
    featureTypeLines.length ? '  interface TeamplayFeatures extends __TeamplayFeatures {}' : null,
    pluginOptionLines.length ? '  interface TeamplayPluginOptions {' : null,
    ...pluginOptionLines,
    pluginOptionLines.length ? '  }' : null,
...
function buildTeamplayTypeImportLine ({ manifestLines, helperTypes, fieldLines, hasPrivateCollections }) {
  const imports = []
  if (manifestLines.length) imports.push('type CollectionsFromManifest')
  if (helperTypes.length) imports.push('type FromJsonSchema')
  if (manifestLines.length) imports.push('type PathModelsFromManifest')
  if (hasPrivateCollections) imports.push('type PrivateCollectionsFromManifest')
  if (fieldLines.length) imports.push('type SignalChild')
  if (!imports.length) return
  return `import { ${imports.join(', ')} } from 'teamplay'`
}
...
function toPrivateTypeName (collectionName) {
  return toTypeName(String(collectionName).replace(/^[$_]+/, ''))
}
