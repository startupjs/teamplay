...
- **Client-only (default)**: in-memory only, no server, no persistence across reloads.
...
By default, TeamPlay runs in client-only mode. It keeps everything in memory, so your data is not persisted between page reloads. If you want a client-only setup with local persistence, use the [Offline Mode](/guide/offline-mode).
